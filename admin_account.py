...
def get_user_details(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT fullname, username, phonenumber FROM users WHERE id=?", (user_id,))
    user_details = cursor.fetchone()
    conn.close()
    return user_details

def open_store_insights():
    df = pd.read_csv(r"/Users/kanithasem/python test/.vscode/final_project/customers.csv")
    drinks = df["Drinks"]
    customers = df["Customers Name"]
    plt.bar(customers, drinks, color="blue", label="")
    plt.xlabel("Customers")
    plt.ylabel("Drinks")
    plt.xticks(rotation=10)
    plt.title("Customer Insight")
    plt.legend()
    plt.show()
...
def create_account_page(master):
    ...
    # Back button
    # back_button = tk.Button(upper_frame, text="< Back", font=("Times New Roman", 12), bg="#878378", fg="black", command=account_window.destroy)
    # back_button.pack(side="left", padx=10, pady=10)

    tk.Label(upper_frame, text="Account Setting", font=("Times New Roman", 20, "bold"), bg="#878378", fg="black").pack(pady=15)

    if logged_in_user_id is not None:
        user_info = get_user_details(logged_in_user_id)
        if user_info:
            ...
            insights_button = tk.Button(account_window, text="View Store Insights", font=("Times New Roman", 14), command=open_store_insights, bg="#878378", fg="black", padx=20, pady=10)
            insights_button.pack(pady=20)
